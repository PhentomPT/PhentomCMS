...
if (isset($_POST['rusername']) and isset($_POST['rpassword'])){
	if ($_POST['rpassword'] == $_POST['vpassword']){
		...
		$check = $mysqli -> query("SELECT * FROM 'account' WHERE username='".$username."'") or die($mysqli -> error); 
		...
		if ($count > 0){
			...
			$mysqli -> query("INSERT INTO 'account' ('username', 'sha_pass_hash', 'email', 'expansion') VALUES ('".$username."','".$sha_pass."','".$email."',".$trinity_expansion.")") or die($mysqli -> error);
